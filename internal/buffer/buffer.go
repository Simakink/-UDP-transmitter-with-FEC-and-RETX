...
func NewBufferedFileWriterWithWorkersAndDirectIO(filename string, bufferSize int, numWorkers int, directIO bool) (*BufferedFileWriter, error) {
	...
	if directIO {
		...
		if err != nil {
			log.Printf("[BufferedWriter] Direct I/O not supported, falling back to standard I/O: %v", err)
...
func (w *BufferedFileWriter) writeWorker() {
	...
	for {
		...
		case data, ok := <-w.writeChan:
			...
			if err != nil {
				log.Printf("[BufferedWriter] Write error: %v", err)
			} else {
				// Update metrics
...
func (rb *ReorderBuffer) Add(pkt *packet.Packet) {
	...
	if pkt.PacketID == config.EndOfStreamMarker && string(pkt.Payload) == "END_OF_STREAM" {
		utils.DebugLog("Received END_OF_STREAM marker. Last chunk should be: %d", pkt.ChunkID-1)
	...
	if !ok {
		...
		if pkt.ChunkID < rb.expected {
			rb.mu.Unlock()
			utils.DebugLog("LATE: Ignoring late packet %d for already processed chunk %d (expected: %d)",
		...
		utils.DebugLog("NEW: Created new ChunkBuffer for chunk %d (k=%d r=%d total=%d)",
			pkt.ChunkID, cb.k, cb.r, cb.Total)
	}
	...
	if _, exists := cb.Packets[pkt.PacketID]; !exists {
		...
		utils.DebugLog("RX: Chunk %d: received packet %d, total received: %d/%d",
			pkt.ChunkID, pkt.PacketID, cb.Got, cb.Total)
	} else {
		utils.DebugLog("DUP: Chunk %d: duplicate packet %d ignored", pkt.ChunkID, pkt.PacketID)
		return
	}
...
func (rb *ReorderBuffer) handleEndOfStream(lastChunkID uint64) {
	...
	log.Printf("[CLIENT] End of stream detected. Last chunk: %d, currently expected: %d", lastChunkID, rb.expected)
	...
	if rb.expected > lastChunkID {
		log.Println("[CLIENT] All chunks processed. Stream complete.")
...
func (rb *ReorderBuffer) waitForCompletion() {
	...
	for {
		select {
		case <-timeout.C:
			log.Printf("[CLIENT] Completion timeout. Expected: %d, Last: %d. Forcing shutdown.",
		...
		case <-ticker.C:
			...
			log.Printf("[CLIENT] Waiting for completion. Expected: %d, Last: %d, Missing chunks: %d",
...
func (rb *ReorderBuffer) tryXORRecovery(cb *ChunkBuffer) {
	...
	if len(missing) == 1 {
		...
		utils.DebugLog("[FEC XOR] recovered packet %d for chunk %d", missing[0], cb.ID)
	}
...
func (rb *ReorderBuffer) tryRSRecovery(cb *ChunkBuffer, pkt *packet.Packet) {
	utils.DebugLog("RS: Processing chunk %d, packet %d, rsDone=%v, rsPresent=%d/%d",
	...
	if cb.rsEnc == nil {
		utils.DebugLog("RS: rsEnc not initialized for chunk %d", cb.ID)
		return
	}
	...
	if int(pkt.PacketID) < cb.k+cb.r {
		...
		if cb.rsShards[pkt.PacketID] == nil {
			...
			utils.DebugLog("RS: Added shard %d for chunk %d, present=%d/%d",
	...
	if cb.rsPresent >= cb.k {
		utils.DebugLog("RS: Attempting reconstruction for chunk %d", cb.ID)
		...
		if err := cb.rsEnc.Reconstruct(cb.rsShards); err == nil {
			...
			if recovered > 0 {
				...
				utils.DebugLog("[FEC RS] recovered %d packets for chunk %d", recovered, cb.ID)
			}
			...
			rb.flushReadyLocked()
			utils.DebugLog("RS: Chunk %d marked as ready after recovery", cb.ID)
		} else {
			utils.DebugLog("[FEC RS] reconstruction failed for chunk %d: %v", cb.ID, err)
		}
	}
}
...
func (rb *ReorderBuffer) checkRetxTimeout() {
	...
	if ok && !exp.Ready && time.Since(exp.createdAt) > rb.timeout {
		var missing []uint32
		...
		if len(missing) > 0 && rb.retxReq != nil {
			log.Printf("[RETX] request chunk %d missing %v", rb.expected, missing)
...
func (rb *ReorderBuffer) flushReadyLocked() {
	utils.DebugLog("FLUSH: Starting flush, expected=%d, total chunks=%d", rb.expected, len(rb.chunks))

	for {
		cb, ok := rb.chunks[rb.expected]
		if !ok {
			utils.DebugLog("FLUSH: Chunk %d not found, stopping flush", rb.expected)
			return
		}
		if !cb.Ready {
			utils.DebugLog("FLUSH: Chunk %d not ready (%d/%d packets, ready=%v), stopping flush",
				rb.expected, cb.Got, cb.Total, cb.Ready)
			return
		}

		utils.DebugLog("FLUSH: Processing chunk %d (%d/%d packets, ready=%v)",
			rb.expected, cb.Got, cb.Total, cb.Ready)
		...
		utils.DebugLog("FLUSH: Deleted chunk %d from buffer", rb.expected)
		rb.expected++
		utils.DebugLog("FLUSH: Next expected chunk: %d", rb.expected)
	}
}

// periodicCleanup runs in background to handle edge cases and cleanup
func (rb *ReorderBuffer) periodicCleanup() {
	ticker := time.NewTicker(5 * time.Second) // Run every 5 seconds for better accumulation
	defer ticker.Stop()
...
func (rb *ReorderBuffer) writeChunkDataDirect(cb *ChunkBuffer) {
	...
	for i := 0; i < dataCount && written < limit; i++ {
		if p, ok := cb.Packets[uint32(i)]; ok {
			...
			log.Printf("missing data packet in chunk %d id %d", cb.ID, i)
		}
	}
	...
	if len(allData) > 0 {
		...
		if err != nil {
			log.Printf("write error for chunk %d: %v", cb.ID, err)
			return
		}
		utils.DebugLog("WRITE: Successfully wrote %d bytes for chunk %d", n, cb.ID)
	} else {
		utils.DebugLog("WRITE: No data to write for chunk %d", cb.ID)
	}

	if written != limit {
		log.Printf("WARNING: chunk %d incomplete: written %d, expected %d", cb.ID, written, limit)
	} else {
		utils.DebugLog("WRITE: Chunk %d completed: written %d bytes", cb.ID, written)
	}
